...
[3A] 2026-04-20 — Task A6~A9 완료 — GameRegistry · Upload API · GameRunner · Socket.io attachIo.
  - A6: GameRegistry(games/ 스캔, meta 파싱, chokidar 감시) + GameMetaSchema zod 추가 + tests/registry.test.ts 3종.
  - A7: upload.ts(multer 256KB) + routes/games.ts(GET/POST) + buildApp에 registry/gamesDir 통합.
  - A8: GameRunner(max/min 패자 결정, 동점 랜덤, 7 tests) — runner.test.ts 완비.
