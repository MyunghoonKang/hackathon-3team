...
식후 최대 8명이 본인 노트북에서 각자 iframe 게임을 플레이한다. 게임은 **단일 HTML 파일** 한 장이며, 본인 결과값(숫자 하나)만 서버로 보낸다. 서버가 전체 값을 비교해 패자 1명을 뽑는다. 패자가 ERP 품의서를 올리게 된다.
...
2. 호스트가 게임 선택 → "시작!" → 최대 8명 모두 GameView 로 전환 → 플레이 → 패자 발표.
